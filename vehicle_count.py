...
from weather import dehaze

# Initialize Tracker
tracker = EuclideanDistTracker()

# Initialize the videocapture object
cap = cv2.VideoCapture(r'dataset\videos\video6.mp4')
input_size = 320
...
# uncomment this when running offline
...
def realTime():
    while True:
        ...
        weather = weather_predictor.detect(img)

        if weather == 0: # rainy
            # add code to remove rain
            print('rainy')
        elif weather == 1 or weather == 2: # normal
            print('normal')
        elif weather == 3: # Snowy
            # add code to remove snow
            print('snowy')
        elif weather == 4: # Foggy
            # add code to remove fog
            print('Foggy')
        ...
        cv2.putText(img, "Car:        "+str(up_list[0] + down_list[0]), (20, 40), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
        cv2.putText(img, "Motorbike:  "+str(up_list[1] + down_list[1]), (20, 60), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
        cv2.putText(img, "Bus:        "+str(up_list[2] + down_list[2]), (20, 80), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
        cv2.putText(img, "Truck:      "+str(up_list[3] + down_list[3]), (20, 100), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
...
if __name__ == '__main__':
    ...
    # img = cv2.imread(image_file)
    # op = from_static_image(dehaze.dehaze(img))
    # cv2.imshow("image", op)
    # cv2.waitKey(0)

    # cv2.imwrite('op.png', ip)
    # w = weather_predictor.detect(img)

    # if(w == 4):
        # print('Foggy')
        # img = cv2.imread(image_file)
        # from_static_image(dehaze.dehaze(img))